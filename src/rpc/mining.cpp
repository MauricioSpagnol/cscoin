...
//        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Flux is downloading blocks...");
    ...
    for (auto payout : mapFluxnodePayouts) {
        std::string tier_name = "proton";

        if      (payout.first == ELETRON) tier_name = "eletron";
        else if (payout.first == NEUTRON) tier_name = "neutron";
        else if (payout.first == FOTON)   tier_name = "foton";

        CTxDestination dest;
        ExtractDestination(payout.second.first, dest);

        result.pushKV(std::string(tier_name + "_csnode_address"), EncodeDestination(dest));
        result.pushKV(std::string(tier_name + "_csnode_payout"),  payout.second.second);
    }

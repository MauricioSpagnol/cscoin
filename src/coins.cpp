...
void InitializeCoinTierAmounts() {
    ...
    mapCoinTierPercentages[PROTON]  = V1_CSNODE_PERCENT_PROTON;
    mapCoinTierPercentages[ELETRON] = V1_CSNODE_PERCENT_ELETRON;
    mapCoinTierPercentages[NEUTRON] = V1_CSNODE_PERCENT_NEUTRON;
    mapCoinTierPercentages[FOTON]   = V1_CSNODE_PERCENT_FOTON;
...
std::set<CAmount> GetCoinAmountsByTier(const int& p_Height, const int& nTier)
{
    ...
    if (nTier == PROTON) {
        setAmounts.insert(V1_CSNODE_COLLAT_PROTON * COIN);
    } else if (nTier == ELETRON) {
        setAmounts.insert(V1_CSNODE_COLLAT_ELETRON * COIN);
    } else if (nTier == NEUTRON) {
        setAmounts.insert(V1_CSNODE_COLLAT_NEUTRON * COIN);
    } else if (nTier == FOTON) {
        setAmounts.insert(V1_CSNODE_COLLAT_FOTON * COIN);
    }

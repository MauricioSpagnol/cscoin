...
#include "csnode/csnodecachedb.h"
        ...
        if (IsDevFundInterval(nHeight)) {
            ...
            if (!fFoundDevFund) {
                return state.DoS(dosLevel, error("ContextualCheckTransaction(): dev fund not correct in coinbase"),
                                 REJECT_INVALID, "tx-coinbase-missing-dev-fund");
            }
        }

        // Check for marketing fund
        if (IsMarketingFundInterval(nHeight)) {
            CAmount nMarketingFundAmount = Params().GetMarketingFundAmount();
            bool fFoundMarketingFund = false;
            for (const auto& out : tx.vout) {
                if (out.scriptPubKey == GetScriptForDestination(DecodeDestination(Params().GetMarketingFundAddress()))) {
                    if (out.nValue == nMarketingFundAmount) {
                        fFoundMarketingFund = true;
                        break;
                    }
                }
            }
            if (!fFoundMarketingFund) {
                return state.DoS(dosLevel, error("ContextualCheckTransaction(): marketing fund not correct in coinbase"),
                                 REJECT_INVALID, "tx-coinbase-missing-marketing-fund");
            }
        }
    }
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // CS Coin emission policy: halvings occur normally for the first two periods
    // (150 CS -> 75 CS -> 37.5 CS), then the reward stabilizes permanently at 37.5 CS.
    // Rationale: reward is split across 5 categories (miners + 4 node tiers), lost
    // wallets and network fees provide natural deflation without requiring further cuts.
    if (halvings >= 2) {
        nSubsidy >>= 2;
        return nSubsidy;
    }
...
bool IsDevFundInterval(const int64_t nHeight) {
    const int64_t nStartHeight = Params().GetDevFundStartHeight();
    const int nInterval = Params().GetDevFundInterval();
    const int nMaxTimes = Params().GetDevFundMaxTimes();
...
bool IsMarketingFundInterval(const int64_t nHeight) {
    const int64_t nStartHeight = Params().GetMarketingFundStartHeight();
    const int nInterval = Params().GetMarketingFundInterval();
    const int nMaxTimes = Params().GetMarketingFundMaxTimes();
...
void static InvalidChainFound(CBlockIndex* pindexNew, const CChainParams& chainParams)
{
    ...
    if (!tip) {
        LogPrintf("%s: no active chain tip (genesis block failed validation)\n", __func__);
        return;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck, FluxnodeCache* p_fluxnodeCache)
{
    ...
    // (genesis block handled above — this case is now unreachable for genesis)
    if (block.GetHash() == chainparams.GetConsensus().hashGenesisBlock) {
        if (!fJustCheck) {
            view.SetBestBlock(pindex->GetBlockHash());
    ...
    CAmount devFund = IsDevFundInterval(pindex->nHeight) ? chainparams.GetDevFundAmount() : 0;
    CAmount marketingFund = IsMarketingFundInterval(pindex->nHeight) ? chainparams.GetMarketingFundAmount() : 0;
    if (pindex->nHeight > 2 && block.vtx[0].GetValueOut() > blockReward + exchangeFund + swapPoolFund + foundationFund + devFund + marketingFund)
        return state.DoS(100,
    ...
    const CChainParams& chainparams, CBlockIndex * const pindexPrev, bool fFromAccept)
{
    ...
    // Genesis block: coinbase is pre-Overwinter (Sprout) format and is unspendable.
    // Skip transaction contextual checks, consistent with ConnectBlock's genesis shortcut.
    if (pindexPrev == NULL)
        return true;

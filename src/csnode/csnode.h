 ...
 * This will be the place that will hold all csnode collateral amounts
 */
// CSNode collateral amounts (fixed — no collateral reduction planned)
#define V1_CSNODE_COLLAT_PROTON  10000
#define V1_CSNODE_COLLAT_ELETRON 25000
...
enum Tier {
    ...
    NEUTRON = 3,
    FOTON   = 4,
    LAST    = 5 // All newly added Tier must be added above LAST, and change the assigned values so they are in order
};

// Legacy aliases — kept so serialized data (nTier 1/2/3) remains compatible
static const int CUMULUS = PROTON;
static const int NIMBUS  = ELETRON;
static const int STRATUS = NEUTRON;
...
public:
    ...
    bool isProton()  { return nTier == PROTON; }
    bool isEletron() { return nTier == ELETRON; }
    bool isNeutron() { return nTier == NEUTRON; }
    bool isFoton()   { return nTier == FOTON; }

    // Legacy aliases
    bool isCumulus() { return isProton(); }
    bool isNimbus()  { return isEletron(); }
    bool isStratus() { return isNeutron(); }
    ...
    std::string TierToString() const
    {
        ...
        if      (nTier == PROTON)  strStatus = "PROTON";
        else if (nTier == ELETRON) strStatus = "ELETRON";
        else if (nTier == NEUTRON) strStatus = "NEUTRON";
        else if (nTier == FOTON)   strStatus = "FOTON";

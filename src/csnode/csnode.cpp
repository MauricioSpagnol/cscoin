...
std::string TierToString(int tier)
{
    ...
    if      (tier == PROTON)  strStatus = "PROTON";
    else if (tier == ELETRON) strStatus = "ELETRON";
    else if (tier == NEUTRON) strStatus = "NEUTRON";
    else if (tier == FOTON)   strStatus = "FOTON";
...
{
    // CS Coin has no collateral migration — V1 collaterals are fixed permanently
    return false;
}
...
void GetUndoDataForExpiredConfirmFluxnodes(CFluxnodeTxBlockUndo& p_fluxnodeTxUndoData, const int& p_nHeight, const std::set<COutPoint> setSpentOuts)
{
    ...
    // CS Coin has fixed V1 collaterals with no transition period — all tiers always enforced
    set<Tier> enforceTiers;
    enforceTiers.insert(PROTON);
    enforceTiers.insert(ELETRON);
    enforceTiers.insert(NEUTRON);
    enforceTiers.insert(FOTON);

    // Get set of valid collateral amounts
    set<CAmount> validAmounts;
    for (int currentTier = PROTON; currentTier != LAST; currentTier++) {
    ...
    for (const auto& item : g_fluxnodeCache.mapConfirmedFluxnodeData) {
        // Enforce valid collateral amounts for all tiers

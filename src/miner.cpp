...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn, std::map<int, std::pair<CScript, CAmount>>* fluxnodePayouts)
{
...

    {
        ...
        // Dev Fund
        if (IsDevFundInterval(pindexPrev->nHeight + 1)) {
            CTxDestination devFundDestination = DecodeDestination(Params().GetDevFundAddress());
            CAmount nAmount = Params().GetDevFundAmount();

            int currentSize = txNew.vout.size();
            int newSize = currentSize + 1;
            txNew.vout.resize(newSize);
            txNew.vout[newSize -1].nValue = nAmount;
            txNew.vout[newSize -1].scriptPubKey = GetScriptForDestination(devFundDestination);
        }

        // Marketing Fund
        if (IsMarketingFundInterval(pindexPrev->nHeight + 1)) {
            CTxDestination marketingFundDestination = DecodeDestination(Params().GetMarketingFundAddress());
            CAmount nAmount = Params().GetMarketingFundAmount();
